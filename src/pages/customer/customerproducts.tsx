...
export default function CustomerProducts() {
  ...
  const enquireMutation = useMutation({
    mutationFn: (product: any) => api.post("/leads", {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((product: any) => (
          <div key={product.id} className="bg-card border border-border rounded-xl p-5">
            ...
            <button onClick={() => enquireMutation.mutate(product)} className="w-full flex items-center justify-center gap-2 py-2 bg-primary text-primary-foreground rounded-xl text-xs font-semibold hover:opacity-90 transition-opacity">
