...
export default function AdminTelecallers() {
  ...
  const { data: tcStats = {} } = useQuery({
    queryKey: ["tc-stats"],
    queryFn: async () => {
      const [leads, calls] = await Promise.all([api.get("/leads"), api.get("/calls")]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {(telecallers as any[]).length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">No telecallers yet</td></tr>}
            {(telecallers as any[]).map((tc: any) => {
              const s = (tcStats as any)[tc.id] || { assigned: 0, callsToday: 0, converted: 0 };
              return (
                <tr key={tc.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
...
function AddTelecallerModal({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api.post("/users", { full_name: form.name, email: form.email, phone: form.phone, password: form.password, role: "telecaller" }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-telecallers"] }); toast.success("Telecaller added"); onClose(); },
