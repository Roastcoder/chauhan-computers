  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {[{ key: "all", label: "All" }, { key: "new", label: "New" }, { key: "followup", label: "Follow-up" }, { key: "not_called", label: "Not Called" }].map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)} className={`px-3 md:px-4 py-2 rounded-xl text-xs font-semibold transition-colors whitespace-nowrap ${filter === f.key ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"}`}>{f.label}</button>
        ))}
      </div>
            ...
            {filtered.map((lead: any) => (
              <tr key={lead.id} className="border-b border-border/50 hover:bg-muted/50">
                ...
                <td className="px-4 py-3">
                  <span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${lead.status === "new" ? "bg-primary/20 text-primary" : lead.status === "contacted" ? "bg-yellow-500/20 text-yellow-500" : lead.status === "interested" ? "bg-purple-500/20 text-purple-500" : lead.status === "converted" ? "bg-green-500/20 text-green-500" : "bg-destructive/20 text-destructive"}`}>{lead.status.replace("_", " ")}</span>
                </td>
        ...
        {filtered.map((lead: any) => (
          <div key={lead.id} className="bg-card border border-border rounded-xl p-4">
            <div className="flex items-start justify-between mb-2">
              ...
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${lead.status === "new" ? "bg-primary/20 text-primary" : lead.status === "converted" ? "bg-green-500/20 text-green-500" : "bg-muted text-muted-foreground"}`}>{lead.status.replace("_", " ")}</span>
            </div>
            {lead.product_interest && <p className="text-[10px] text-muted-foreground mb-2">Interest: {lead.product_interest}</p>}
            <button onClick={() => setCallingLead(lead)} className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-green-600 text-white rounded-lg text-xs font-semibold">
              <Phone className="w-3 h-3" /> Call Now
            </button>
          </div>
        ))}
...
function CallingModal({ lead, onClose }: { lead: any; onClose: () => void }) {
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    setSaving(true);
    await api.post("/calls", { lead_id: lead.id, outcome, remarks, follow_up_at: followUp || null });
    if (remarks) await api.post("/remarks", { lead_id: lead.id, remark: remarks, remark_type: "call" });
    const updates: any = { status };
    if (followUp) updates.follow_up_at = followUp;
    await api.put(`/leads/${lead.id}`, updates);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 flex items-end md:items-center justify-center p-0 md:p-6" onClick={onClose}>
      <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="bg-card border border-border rounded-t-2xl md:rounded-2xl p-5 md:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-3 mb-6">
          <a href={`tel:${lead.phone}`} className="flex-1 flex items-center justify-center gap-2 py-3 bg-green-600 text-white rounded-xl font-semibold text-sm"><Phone className="w-4 h-4" /> Call</a>
          <a href={`https://wa.me/91${lead.phone.replace(/\D/g, "")}?text=${encodeURIComponent("Hi, I'm calling from Chauhan Computers")}`} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 py-3 bg-green-500 text-white rounded-xl font-semibold text-sm"><MessageCircle className="w-4 h-4" /> WhatsApp</a>
        </div>
        <div className="space-y-3">
          ...
          <div>
            ...
            <select value={status} onChange={e => setStatus(e.target.value)} className="w-full px-4 py-2.5 bg-background rounded-xl text-sm text-foreground border border-border outline-none">
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold disabled:opacity-50">{saving ? "Saving..." : "Save"}</button>
        </div>
      </motion.div>
    </motion.div>
  );
