...
export default function AdminReports() {
  ...
  const { data: allLeads = [] } = useQuery({ queryKey: ["report-leads"], queryFn: () => api.get("/leads") });
  const { data: allCalls = [] } = useQuery({ queryKey: ["report-calls"], queryFn: () => api.get("/calls") });

  const leads = (allLeads as any[]).filter((l: any) => l.created_at >= dateFrom && l.created_at <= dateTo + "T23:59:59");
  const calls = (allCalls as any[]).filter((c: any) => c.created_at >= dateFrom && c.created_at <= dateTo + "T23:59:59");

  const sourceBreakdown = ["meta", "google", "manual", "website", "whatsapp"].map(s => ({ source: s, count: leads.filter((l: any) => l.source === s).length }));
  ...
  const exportCsv = () => {
    ...
    const rows = leads.map((l: any) => `${l.name},${l.phone},${l.email || ""},${l.source},${l.status},${l.product_interest || ""},${l.created_at}`).join("\n");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-xl p-5"><p className="text-xs text-muted-foreground">Total Leads</p><p className="text-3xl font-bold text-foreground">{leads.length}</p></div>
        <div className="bg-card border border-border rounded-xl p-5"><p className="text-xs text-muted-foreground">Total Calls</p><p className="text-3xl font-bold text-foreground">{calls.length}</p></div>
        <div className="bg-card border border-border rounded-xl p-5"><p className="text-xs text-muted-foreground">Conversion Rate</p><p className="text-3xl font-bold text-green-400">{conversionRate}%</p></div>
      </div>
