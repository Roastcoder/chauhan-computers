...
export default function TelecallerFollowups() {
  ...
  const withFollowup = (leads as any[]).filter(l => l.follow_up_at);
  const today = new Date().toDateString();
  const todayFollowups = withFollowup.filter(l => new Date(l.follow_up_at).toDateString() === today);
  const upcoming = withFollowup.filter(l => new Date(l.follow_up_at) > new Date());
