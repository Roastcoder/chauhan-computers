...
export default function AdminLeads() {
  ...
  const { data: telecallers = [] } = useQuery({
    ...
    queryFn: () => api.get("/users?role=telecaller"),
  });
  ...
  const assignMutation = useMutation({
    mutationFn: ({ leadId, userId }: { leadId: string; userId: string }) =>
      api.put(`/leads/${leadId}`, { assigned_to: userId }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-leads-list"] }); toast.success("Lead assigned"); },
  });

  const statusMutation = useMutation({
    mutationFn: ({ leadId, status }: { leadId: string; status: string }) =>
      api.put(`/leads/${leadId}`, { status }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-leads-list"] }); toast.success("Status updated"); },
  });

  const deleteMutation = useMutation({
    mutationFn: (leadId: string) => api.delete(`/leads/${leadId}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-leads-list"] }); toast.success("Lead deleted"); },
  });
              ...
              {filtered.map((lead: any) => (
                <tr key={lead.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <select value={lead.status} onChange={e => statusMutation.mutate({ leadId: lead.id, status: e.target.value })} className="text-[10px] font-semibold px-2 py-1 rounded-full bg-background border border-border text-foreground outline-none">
                      {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s.replace("_", " ")}</option>)}
                    </select>
                  </td>
                  <td className="px-4 py-3">
                    <select value={lead.assigned_to || ""} onChange={e => assignMutation.mutate({ leadId: lead.id, userId: e.target.value })} className="text-xs px-2 py-1 bg-background rounded-lg border border-border text-foreground outline-none">
                      <option value="">Unassigned</option>
                      {telecallers.map((t: any) => <option key={t.id} value={t.id}>{t.full_name || t.email}</option>)}
                    </select>
                  </td>
...
function AddLeadModal({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-leads-list"] }); toast.success("Lead added"); onClose(); },
...
function CsvUploadModal({ onClose }: { onClose: () => void }) {
  ...
  const handleImport = async () => {
    const mapped = rows.map(r => ({ name: r.name || r.Name || "", phone: r.phone || r.Phone || "", email: r.email || r.Email || "", product_interest: r.product_interest || r.Product || "", source: "manual" })).filter(r => r.name && r.phone);
    if (mapped.length === 0) { toast.error("No valid rows"); return; }
    await api.post("/leads/bulk", { leads: mapped });
