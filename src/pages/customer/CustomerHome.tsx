...
export default function CustomerHome() {
  ...
  const active = (enquiries as any[]).filter(e => !["converted", "not_interested"].includes(e.status)).length;
  const converted = (enquiries as any[]).filter(e => e.status === "converted").length;
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {[
          ...
          { icon: Clock, label: "Total Enquiries", value: (enquiries as any[]).length, color: "text-cyan-500" },
          { icon: Star, label: "Reward Points", value: points, color: "text-yellow-500" },
        ].map((kpi, i) => (
          <motion.div key={kpi.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card border border-border rounded-xl p-4 md:p-5">
      ...
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 rounded-xl p-4 md:p-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">Earn points on every purchase and redeem as discount on your next order.</p>
        {loyaltyPoints && (
          <div className="flex gap-4 text-xs text-muted-foreground">
            <span>Earned: <strong className="text-foreground">{(loyaltyPoints as any).lifetime_earned}</strong></span>
            <span>Redeemed: <strong className="text-foreground">{(loyaltyPoints as any).lifetime_redeemed}</strong></span>
          </div>
        )}
      </div>
      ...
      {(recentTransactions as any[]).length > 0 && (
        <div className="bg-card border border-border rounded-xl p-4 md:p-6">
          ...
          <div className="space-y-2">
            {(recentTransactions as any[]).map((txn: any) => (
              <div key={txn.id} className="flex items-center justify-between bg-background rounded-xl p-3">
                ...
                <span className={`text-sm font-bold ${txn.type === "earn" ? "text-green-500" : "text-destructive"}`}>{txn.type === "earn" ? "+" : "-"}{txn.points}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Link to="/customer/products" className="bg-card border border-border rounded-xl p-4 flex items-center justify-between hover:bg-muted/50 transition-colors">
          <div className="flex items-center gap-3"><ShoppingBag className="w-5 h-5 text-primary" /><span className="text-sm font-medium text-foreground">Browse Products</span></div>
        ...
        <Link to="/customer/quote" className="bg-card border border-border rounded-xl p-4 flex items-center justify-between hover:bg-muted/50 transition-colors">
          <div className="flex items-center gap-3"><FileText className="w-5 h-5 text-primary" /><span className="text-sm font-medium text-foreground">Request a Quote</span></div>
          <ArrowRight className="w-4 h-4 text-muted-foreground" />
        </Link>
      </div>
    </div>
